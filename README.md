...
| **Focus Zone** | Prayer checklist + lock, the Entertainment Zone gate (Instagram/Facebook/LinkedIn), the Research box, and the Daily Update AI-prompt box — see below |

## Focus Tasks (Dashboard)

Four task groups, all visible together as cards on the Dashboard:

- **Daily Musts** — fixed daily obligations (prayers, Quran, etc). Resets every Fajr, and tracks a streak (consecutive days where every item got checked).
- **Top 3 Mandatory** — exactly 3 tasks you set first thing each day. These are what gate the Entertainment Zone below. Anything left unchecked at day's end carries over to tomorrow instead of disappearing.
- **Important** — deadline-based tasks, sorted by nearest deadline first. Miss the deadline and it's marked **Overdue** and pinned to the top instead of being removed.
- **Optional / Interests** — a plain backlog, no deadline, no pressure.

Add from any of the four via the quick-add bar right above the cards (pick a category, add an optional one-line "why"). A **Weekly Review** panel at the bottom of the Dashboard summarizes the week's Daily Musts completion rate, currently-overdue Important tasks, and total Entertainment Zone time.

## Focus Zone

- **🕌 Prayer Times** — pulled once a day from the Aladhan API for Chattogram, Bangladesh (method 1). Tick each of the five prayers after you pray. Miss a window (the next prayer's start time arrives, unticked) and the Entertainment Zone + Research box lock until the next prayer time — the rest of the app stays open.
- **📵 Entertainment Zone** — Instagram / Facebook / LinkedIn tiles, gated by how many of today's Top 3 you've completed: 0 → locked, 1 → 5 min, 2 → 10 min, 3 → a 60-minute daily budget (minus whatever you've already used today from the smaller tiers — so finishing all three after already using the 5 + 10 min windows nets 45 usable minutes, but finishing them fresh nets the full 60). Total time across all sessions in a day is capped at 2 hours, and both the daily cap and every lock reset at Fajr, not midnight. Once a session starts you get a visible countdown; let it hit zero without closing the tab and a full-screen overlay covers the screen (with a sound) until you confirm you've closed it — the overage (capped at 30 min) gets added to a punitive 1-hour lockout before you can open the zone again.
- **🔎 Research** — opens a Google search in a new tab. No timer, no lock — just a one-line "reason for searching" logged before it opens, so goal-directed searches stay easy to tell apart from browsing later.

All of this is enforced at the UI/JS level only — a website can't force-close another tab or lock the OS screen. The countdown, the overlay, and the disabled tiles are the actual enforcement; nothing here stops opening Instagram in an unrelated tab. That limitation is by design.
