...
const defaultState = () => ({
  ...
  // ---- Focus & Access Control additions ----
  focus: {
    dailyMusts: [],          // { id, text, done, why, createdAt }
    dailyMustsDayKey: null,  // the fajr-day these done-flags belong to
    dailyMustsStreak: 0,
    dailyMustsHistory: [],   // [{ dayKey, total, completed }] — last ~30 days, for weekly review
    top3: [],                 // up to 3: { id, text, done, why, createdAt, carried }
    top3DayKey: null,
    important: [],            // { id, text, deadline (ISO 'YYYY-MM-DDTHH:MM'), done, why, createdAt, overdue }
    optional: []               // { id, text, why, createdAt }
  },
  entertainment: {
    usedTodayMin: 0,     // cumulative minutes consumed this fajr-day, incl. overage
    dayKey: null,
    negBalanceMin: 0,    // minutes of overage owed, folded into the next lockout when the overlay is dismissed
    lockedUntil: null,   // epoch ms — punitive lockout from a past overage session
    session: null,        // { platform, startedAt, allowedSec }
    usageHistory: []       // [{ dayKey, minutes }] — last ~30 days, for weekly review
  },
  prayer: {
    dayKey: null,
    timings: null,   // { Fajr, Dhuhr, Asr, Maghrib, Isha } as "HH:MM"
    fetchedAt: null,
    checked: {}       // { Fajr:true, ... } for the current dayKey
  },
  research: {
    log: []  // { id, query, reason, at }
  }
});
...
function normalizeState(parsed) {
  ...
  return {
    ...
    entertainment: { ...base.entertainment, ...(parsed.entertainment || {}) },
    prayer: { ...base.prayer, ...(parsed.prayer || {}) },
    research: { ...base.research, ...(parsed.research || {}) }
  };
}

// ---- Fajr-day boundary helper ----
// The user's "day" runs Fajr-to-Fajr rather than midnight-to-midnight. Given today's Fajr
// time (as "HH:MM", from the cached Aladhan response) and the current moment, returns the
// date-string key for which fajr-day we're currently in: if it's not yet reached today's
// Fajr time, we're still inside *yesterday's* cycle. Falls back to a plain calendar-day key
// if no Fajr time is cached yet (e.g. before the first successful API fetch).
function fajrDayKey(fajrHHMM, now = new Date()) {
  if (!fajrHHMM) return todayKey(now);
  const [fh, fm] = fajrHHMM.split(':').map(Number);
  const fajrToday = new Date(now.getFullYear(), now.getMonth(), now.getDate(), fh || 0, fm || 0, 0, 0);
  if (now < fajrToday) {
